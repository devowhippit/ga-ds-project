...
function be_scripts() {
	$get_plugin_directory_uri = plugins_url( '' , __FILE__ );
	// CSS
	wp_enqueue_style(  'farbtasticcss' , $get_plugin_directory_uri . '/lib/farbtastic/farbtastic.css'    , null , null , 'screen' );
	// Javascript
	wp_enqueue_script( 'farbtasticjs'  , $get_plugin_directory_uri . '/lib/farbtastic/farbtastic.js'     , null , array( 'jQuery' ) , true );
	wp_enqueue_script( 'raphaeljs'     , '//cdnjs.cloudflare.com/ajax/libs/raphael/2.1.0/raphael-min.js' , null , array( 'jQuery' ) , true );
	wp_enqueue_script( 'morrisjs'      , 'http://cdn.oesmith.co.uk/morris-0.4.3.min.js'                  , null , array( 'jQuery' , 'raphaeljs' ) , true );
	wp_enqueue_script( 'be_mainjs'     , $get_plugin_directory_uri . '/js/main.js'                       , null , array( 'jQuery' , 'raphaeljs' , 'farbtasticjs' ) , true );
}
...
function be_getbehance() {
	...
	$cache_fields      = 'wp-content/cache/be_cache_fields.json';
...
function be_loop( $id , $data ) {
	...
	echo '<pre><small>';
	echo 'ID: '.$id[ 'id' ]."\n";
	echo 'local: <a href="http://'.$_SERVER['SERVER_NAME'].'/wp-content/cache/'.$id[ 'id' ].'.json" target="_blank">'.$_SERVER['SERVER_NAME'].'/wp-content/cache/'.$id[ 'id' ].'</a>'."\n";
	...
	foreach( $data[ 'projects' ] as $project ) :
		...
		$local = 'wp-content/cache/img/' . $cover;
...
function be_buildchart() {
	...
	$( document ).ready( function() {
		Morris.Bar({
			element: '<? echo $id ?>',
			data: [ <?
		foreach ( scandir( 'wp-content/cache' ) as $filename ) :
		    if ( strpos( $filename , $value ) ) :
		    	$data = json_decode( file_get_contents( 'wp-content/cache/' . $filename ) , true );
		    	foreach( $data[ 'projects' ] as $project ) : 
		    		$project_name = str_replace( '"' , '' , $project[ 'name' ] );
					$project_name = preg_replace( "/\r|\n/" , '' , $project_name );
					echo '{ y : "'.$project_name.'" , views : '.$project[ 'stats' ][ 'views' ].' },';
