...
class baw_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'behance-and-wordpress'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'behance-and-wordpress'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'behance-and-wordpress'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'behance-and-wordpress'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'behance-and-wordpress');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'behance-and-wordpress'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'behance-and-wordpress');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'behance-and-wordpress'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'behance-and-wordpress') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'behance-and-wordpress');
            case 'false':
                return __('false', 'behance-and-wordpress');

            case 'Administrator':
                return __('Administrator', 'behance-and-wordpress');
            case 'Editor':
                return __('Editor', 'behance-and-wordpress');
            case 'Author':
                return __('Author', 'behance-and-wordpress');
            case 'Contributor':
                return __('Contributor', 'behance-and-wordpress');
            case 'Subscriber':
                return __('Subscriber', 'behance-and-wordpress');
            case 'Anyone':
                return __('Anyone', 'behance-and-wordpress');
        }
        return $optionValue;
    }
